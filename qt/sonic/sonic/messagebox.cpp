    ...
    m_move_widget(false)
{
    ...
    ui->m_close_btn->setAutoDefault(false);
...
void MessageBox::disableClose(bool disabled)
{
    ui->m_close_btn->setVisible(!disabled);
}
...
QMessageBox::Button MessageBox::showMessageBox(QWidget *parent, QMessageBox::Icon icon,
                                               const QString &title, const QString &text,
                                               QMessageBox::StandardButtons buttons)
{
    MessageBox msg_box(icon, title, text, buttons, parent);
    msg_box.disableClose();
    msg_box.exec();
    return msg_box.standarClickedButton();
}

QMessageBox::Button MessageBox::information(const QString &title, const QString &text,
                                            QMessageBox::StandardButtons buttons, QWidget *parent)
{
    auto real_title = title;
    if (real_title.isEmpty()) {
        real_title = "Information";
    }

    return showMessageBox(parent, QMessageBox::Information, real_title, text, buttons);
}

QMessageBox::Button MessageBox::question(const QString &title, const QString &text,
                                         QMessageBox::StandardButtons buttons, QWidget *parent)
{
    auto real_title = title;
    if (real_title.isEmpty()) {
    ...
    return showMessageBox(parent, QMessageBox::Question, real_title, text, buttons);
}

QMessageBox::Button MessageBox::warning(const QString &title, const QString &text,
                                        QMessageBox::StandardButtons buttons, QWidget *parent)
{
    auto real_title = title;
    if (real_title.isEmpty()) {
    ...
    return showMessageBox(parent, QMessageBox::Warning, real_title, text, buttons);
}

QMessageBox::Button MessageBox::critical(const QString &title, const QString &text,
                                         QMessageBox::StandardButtons buttons, QWidget *parent)
{
    auto real_title = title;
    if (real_title.isEmpty()) {
    ...
    return showMessageBox(parent, QMessageBox::Critical, real_title, text, buttons);
}
...
void MessageBox::keyPressEvent(QKeyEvent *event)
{
    if (event->matches(QKeySequence::Cancel) ||
            ((!event->modifiers() || (event->modifiers() & Qt::KeypadModifier && event->key() == Qt::Key_Enter)) &&
